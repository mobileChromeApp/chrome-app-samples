...
function check(r) {
    ...
    var Max = 20;
    var size = $("#screen").css("font-size").split("px");
    size = size[0] + size[1];
    var bottom = $("#screen").css("bottom").split("px");
	...
	bottom = $("#screen").css("bottom").split("px");
	bottom = bottom[0] + bottom[1];
...
function key(x, y) {
    ...
    case "=":
        calc += $("#screen").html();
		calc = math.eval(calc);
		calc = math.round(parseFloat(parseFloat(calc).toFixed(9)), 9);
		old_calc = calc;

...
function dbV11(d, error, done) {
	d.changeVersion(d.version, '1.1', function (t) {
		t.executeSql('CREATE TABLE styles (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, url TEXT, updateUrl TEXT, md5Url TEXT, name TEXT NOT NULL, image TEXT NOT NULL, code TEXT NOT NULL, enabled INTEGER NOT NULL, originalCode TEXT NULL);');
...
function dbV12(d, error, done) {
	d.changeVersion(d.version, '1.2', function (t) {
		...
		t.executeSql('CREATE TABLE newstyles (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, url TEXT, updateUrl TEXT, md5Url TEXT, name TEXT NOT NULL, image TEXT NOT NULL, enabled INTEGER NOT NULL);');
		t.executeSql('INSERT INTO newstyles (id, url, updateUrl, md5Url, name, image, enabled) SELECT id, url, updateUrl, md5Url, name, image, enabled FROM styles;');
			...
			if ("matchUrl" in options) {
				...
				t.executeSql(sql, matchParams, function (t, r) {
					...
					getGlobalStyleIds(function(ids) {
						...
						loadStyles(style_ids, options.image, options.enabled, options.url, callback);
					});
				});
			} else {
				loadStyles(options.id ? [options.id] : null, options.image, options.enabled, options.url, callback);
			}
		}, reportError);
	}, reportError);
}
...
function getCache(callback) {
	...
	(chrome.extension.sendMessage ? chrome.extension.sendMessage : chrome.extension.sendRequest)({name: "getCachedStyles"}, callback);
}
...
function loadStyles(styleIds, image, enabled, url, callback) {
	...
	if (styleIds) {
		getCache(function(cache) {
			...
			styleIds.forEach(function(id) {
				if (cache[id]) {
					if (checkStyle(cache[id], image, enabled, url)) {
			...
			loadStylesFromDB(styles, styleIds, image, enabled, url, callback);
		});
		return;
	}
	loadStylesFromDB([], styleIds, image, enabled, url, callback);
}

function checkStyle(style, image, enabled, url) {
	return (image == null || image == style.image) && (enabled == null || enabled == fixBoolean(style.enabled)) && (url == null || url == style.url);
}

function loadStylesFromDB(styles, styleIds, image, enabled, url, callback) {
			...
			t.executeSql('SELECT DISTINCT s.*, se.id section_id, se.code, sm.name metaName, sm.value metaValue FROM styles s LEFT JOIN sections se ON se.style_id = s.id LEFT JOIN section_meta sm ON sm.section_id = se.id WHERE 1' + where + ' ORDER BY s.id, se.id, sm.id', params, function (t, r) {
				...
				for (var i = 0; i < r.rows.length; i++) {
					...
					if (currentStyle == null || currentStyle.id != values.id) {
						currentStyle = {id: values.id, url: values.url, updateUrl: values.updateUrl, md5Url: values.md5Url, name: values.name, image: values.image, enabled: values.enabled, sections: []};
				...
				if (isBackground()) {
					...
					(chrome.extension.sendMessage ? chrome.extension.sendMessage : chrome.extension.sendRequest)({name: "cacheStyles", styles: styles});
				}
				callback(styles.filter(function(style) {
					return checkStyle(style, image, enabled, url);
				}));
			}, reportError);
		}, reportError);
	}, reportError);
...
function saveFromJSON(image, o, callback) {
	getDatabase(function(db) {
		db.transaction(function (t) {
			if (o.id) {
				...
				t.executeSql('INSERT INTO styles (name, image, enabled, url, updateUrl) VALUES (?, ?, ?, ?, ?);', [o.name, image, true, o.url, o.updateUrl]);
			}
...
function saveFromJSONComplete(id, callback) {
	(chrome.extension.sendMessage ? chrome.extension.sendMessage : chrome.extension.sendRequest)({name: "styleChanged"});
	if (id) {

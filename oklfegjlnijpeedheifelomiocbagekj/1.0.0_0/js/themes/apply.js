(chrome.extension.sendMessage ? chrome.extension.sendMessage : chrome.extension.sendRequest)({name:"getStylesToApply"}, function(response) {
	...
	switch(request.name) {
		...
		case "styleAdded":
			if (request.style.enabled == "true") {
				(chrome.extension.sendMessage ? chrome.extension.sendMessage : chrome.extension.sendRequest)({name: "getStyleApplies", style: request.style, url: location.href}, function(response) {
					if (response) {
						applyStyle(response);
					}
				});

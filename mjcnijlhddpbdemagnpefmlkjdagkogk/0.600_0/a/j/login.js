...
var PopOver = Class.extend(
{
	...
	show : function(anchor)
	{
		...
		{
			this.object.removeClass('popover-new-topleft popover-new-topright popover-new-bottomleft popover-new-bottomright popover-new-bottom');
	...
	try_left : function()
	{
		...
		targetX = Math.round(this.anchorPosition.left);
		...
		//if (targetX - this.popupSize.width > 0)
		if(this.fits(pos))
		{
			this.position = 'left';
			this.offset = pos;
	...
	try_right : function()
	{
		...
		//if (targetX + this.popupSize.width < this.viewPort.width)
		if(this.fits(pos))
		{
			this.position = 'right';
			this.offset = pos;
...
function createDialog(params)
{
	...
	*/
	...
	var cancel = $('<a class="button" href="#">' + params.cancel.title + '</a>');
	...
	if(params.confirm)
	{
		confirm = $('<a class="button button-important" href="#">' + params.confirm.title + '</a>');
...
(function () {
     ...
     */
    var fetchGUID = function(successCallback, errorCallback) {
        apiRequest({
            path: 'guid',
     ...
     */
    var processLoginSignupResponse = function(data, callback) {
        // data: access_token=sdlkfsldkjfksldf&username=ksdjflaksf&uid=lkasjdflsjdf
        var dataArray = data.split("&");
        var accessToken = dataArray[0].split("=")[1];
        var username = dataArray[1].split("=")[1];
        var uid = dataArray[2].split("=")[1];
     ...
     */
    var login = function(username, pass, callbacks) {
        fetchGUID(function(guid) {
            apiRequest({
                ...
                success: function(data) {
                    ...
                    processLoginSignupResponse(data, callbacks.success);
                },
                error: function() {
     ...
     *                            successfully or if there was an error
     */
    var signup = function(username, password, email, callbacks) {
        fetchGUID(function(guid) {
            apiRequest({
                path: 'signup',
                data: {
    ...
    $(document).ready(function() {
        ...
        $('#login-button-link').click(function(evt) {
            ...
            login(username, password, {
                ...
                error : function(error) {
                    ...
                    if (!navigator.onLine) {
                        error_field.innerHTML = "Login cannot be completed without an internet connection.";
                    }
                    else {
        ...
        $('.button-loginmenu').click(function() {
            $('#pre_content').fadeOut(200, function() {
                $(this).removeClass('active');
                $("#username-field,#password-field").val('');
                $('#content').fadeIn(200,function() { $(this).addClass('active'); $('#username-field').focus(); });
            });
        });
        $('.button-cancel').click(function() {
            $('#content').fadeOut(200, function() {
                $(this).removeClass('active');
                $('#pre_content').fadeIn(200, function() { $(this).addClass('active'); });
            });
        });
        $('#username-field,#password-field').keypress(function(e) {
            if (e.keyCode == 13) {
                e.preventDefault();
                $('#login-button-link').trigger('click');
            }
        });
    });
}());